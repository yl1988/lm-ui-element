import {isvalidNan, isvalidPhone, isvalidCardNo, isvalidBankNo} from "./validate.js";
// 严重是否是数字
...
export const validIDCard = (rule, value, callback) => {
    if (!value) {
        ...
        let max = now - 365 * 24 * 60 * 60 * 1000 * 81 //年龄最大80岁
        let min = now - 365 * 24 * 60 * 60 * 1000 * 15 //年龄最小15岁
        if (birthdayDateTime > min || birthdayDateTime < max) {
            callback(new Error("请输入年龄在15岁到80岁之间身份证号码"))
            return
        }
        callback()
    }
...
//检查值是不是下标类型
export const validIsIndex = (rule, value, callback, msg = '未选择该值') => {
    value = Number(value)
    console.log(value)
...
//数组校验
export const validArr = (rule, value, callback, length, msg = '未选择该值') => {
    console.log(value)
...
export const validateURL = (rule, value, callback) => {
    ...
    const noHeadRegex = /^(\/\/)?[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\]@!\$&'\*\+,;=.]+$/
    if (!value){
        callback(new Error("请输入URL"))
    }else if (!strRegex.test(value) && !noHeadRegex.test(value)){
...
export const validateRole=(rule, value, callback,)=>{
    ...
    let regArr=['企业管理员','项目管理员','部门管理员','管理员']
    for(let i=0;i<regArr.length;i++){
        if(new RegExp(`^${regArr[i]}`).test(value)){
            callback(new Error(`不可以添加以  ${regArr[i]}  开头的角色`))
            return
        }
    }
    callback()
